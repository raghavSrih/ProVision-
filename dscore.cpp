...
uint16_t pixelsConfidenceVGA[FORMAT_VGA_PIXELS];
...
uint16_t* pixelsConfidenceVGASnapshot;
...
void onNewDepthSample(DepthNode node, DepthNode::NewSampleReceivedData data)
{
    ...
    {
        if (buildConfidenceFlag) {
            pixelsConfidenceQVGA[currentPixelInd] = data.confidenceMap[currentPixelInd];
        }
...
uint16_t* getPixelsConfidenceVGA() {
    return pixelsConfidenceVGA;
}
...
void configureDepthNode()
{
    ...
    DepthNode::Configuration configRef(frameFormatDepth, frameRateDepth, DepthNode::CAMERA_MODE_LONG_RANGE, true);
                   ...
                   bool buildColorSyncFlag_in, bool buildDepthSyncFlag_in, bool buildConfidenceFlag_in)
{
    ...
    pixelsConfidenceVGASnapshot   = (uint16_t*) malloc(FORMAT_VGA_PIXELS*sizeof(uint16_t));
...
void stop_capture()
{
    ...
    free(pixelsConfidenceVGASnapshot);
